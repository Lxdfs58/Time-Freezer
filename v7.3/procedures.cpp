...
bool init()
{
    ...
    resource::gWindow = SDL_CreateWindow( "TIME FREEZER", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, resource::SCREEN_WIDTH, resource::SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
...
bool loadMedia()
{
    ...
    if( !resource::bluesharkTexture.loadFromFile( "../textures/blueshark.png" ) )
    {
        printf( "Failed to load shark texture!\n" );
        success = false;
    }
    ...
    if( !resource::bluetroodonTexture.loadFromFile( "../textures/bluetroodon.png" ) )
    {
        printf( "Failed to load troodon texture!\n" );
        success = false;
    }
    if( !resource::fishTexture.loadFromFile( "../textures/fish.png" ) )
    {
     ...
     if( !resource::bluefishTexture.loadFromFile( "../textures/bluefish.png" ) )
    {
        printf( "Failed to load fish texture!\n" );
        success = false;
    }
    if( !resource::explosionTexture.loadFromFile( "../textures/Explosion.png" ) )
    {
        printf( "Failed to load explosion texture!\n" );
        success = false;
    }
    if( !resource::gameoverTexture.loadFromFile( "../textures/GAMEOVER.png" ) )
    {
        printf( "Failed to load gameover texture!\n" );
        success = false;
    }
    if( !resource::restartTexture.loadFromFile( "../textures/restart.png" ) )
    {
        printf( "Failed to load restart texture!\n" );
        success = false;
    }
    //Open the font
	resource::gFont = TTF_OpenFont( "../fonts/GenJyuuGothic-Heavy.ttf", 28 );
